...
2. Run `make check`, `make input-check`, `make companion-check`,
   `make performance-check`, `make multitasking-check`, `make adaptive-check`,
   `make device-check`, `make polish-check`, `make integration-fixture`, and
   `make version-check`.
...
5. For the 1.4.0 cut, the `Omanome Release` workflow observes the successful
   `Omanome CI` run on `main`, reads the version from `manifest.json`, creates
   or updates the annotated `v1.4.0` tag on that exact green commit, then
...
The 1.4.0 release is a source release, not a hardware certification claim.
...
```text
tag: v1.4.0 points at the green main commit
release: v1.4.0 is published, not draft or prerelease
assets: omanome-1.4.0.tar.gz and omanome-1.4.0.tar.gz.sha256
archive: contains the source tree without .git metadata
checksum: sha256sum matches the uploaded archive
```

## 1.4 polish release checklist

Before publishing the 1.4.0 tag, confirm that the green main commit contains
the manifest, companion descriptor, native helper metadata, shared UI tokens,
semantic controls, bounded preview teardown, transient-state reset, Device
Graph, Device Profiles, calibration rollback, Hardware Setup Profiles, battery
and quirks safety boundaries, README files, feature matrix, compatibility
snapshot, hardware matrix, and migration/update/recovery documentation.

The release gate must also show the deterministic 100-cycle integration fixture,
the 1.3 → 1.4 configuration preservation test, the executable-mode and UTF-8
source checks, and a clean standard-bar invariant. These are Fixture Tested
portable checks; they are not GUI or physical-device certification.
