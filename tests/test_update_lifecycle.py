...
class UpdateLifecycleTests(unittest.TestCase):
        ...
        (fake_bin / "curl").write_text(
            textwrap.dedent(
                ...
                case "$url" in
                  ...
                  */omanome/v1.4.0/manifest.json) printf '%s\\n' '{"version":"1.4.0"}' ;;
                  *) exit 1 ;;
                esac
                """
            ),
        ...
        (fake_bin / "omarchy").write_text(
            textwrap.dedent(
                ...
                if [[ "$2" == "update" ]]; then
                  ...
                  sed -i 's/"version": "0.9.0"/"version": "1.4.0"/' "$plugin"
                  exit 0
                fi
                if [[ "$2" == "remove" ]]; then
    ...
    def test_stable_channel_uses_release_tag_not_main(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(payload["latestVersion"], "1.4.0")
            args_log = (root / "git-args.log").read_text(encoding="utf-8")
            self.assertIn("refs/tags/v1.4.0^{}", args_log)
    ...
    def test_rollback_restores_pre_migration_1_1_config_without_adaptive_state(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(json.loads((plugin / "manifest.json").read_text(encoding="utf-8"))["version"], "1.4.0")
