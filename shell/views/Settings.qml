    ...
    Flickable {
      ...
      Column {
        ...
        Column {
          ...
          ActionButton { width: parent.width; text: "Advanced compositor effects"; subtitle: root.service.cfg("effects.enabled", true) ? "Native backends may run when available" : "Blur, preview, Wobbly and cube hooks are bypassed"; checked: root.service.cfg("effects.enabled", true); onClicked: root.toggle("effects.enabled") }
          ...
          ActionButton { width: parent.width; text: "Wobbly windows"; subtitle: root.service.statusObject().wobbly.reason; checked: root.service.statusObject().wobbly.enabled; usable: root.service.statusObject().wobbly.available && root.service.cfg("effects.enabled", true); onClicked: root.toggle("wobbly.enabled") }
          Text { text: "Wobbly strength: " + Number(root.service.cfg("wobbly.maxDeformation", 0.035)).toFixed(3); color: Color.foreground; font.pixelSize: Style.font.body }
          Slider { width: parent.width; from: 0.0; to: 0.12; value: root.service.cfg("wobbly.maxDeformation", 0.035); enabled: root.service.statusObject().wobbly.available; onMoved: root.service.setConfig("wobbly.maxDeformation", Math.round(value * 1000) / 1000) }
          Text { text: "Wobbly stiffness: " + Number(root.service.cfg("wobbly.stiffness", 0.72)).toFixed(2) + " · friction: " + Number(root.service.cfg("wobbly.friction", 0.78)).toFixed(2); color: Color.muted; font.pixelSize: Style.font.caption }
          Slider { width: parent.width; from: 0.05; to: 4; value: root.service.cfg("wobbly.stiffness", 0.72); enabled: root.service.statusObject().wobbly.available; onMoved: root.service.setConfig("wobbly.stiffness", Math.round(value * 100) / 100) }
          Slider { width: parent.width; from: 0; to: 4; value: root.service.cfg("wobbly.friction", 0.78); enabled: root.service.statusObject().wobbly.available; onMoved: root.service.setConfig("wobbly.friction", Math.round(value * 100) / 100) }
          Text { text: "Wobbly mesh: " + root.service.cfg("wobbly.gridResolution", 8) + " × " + root.service.cfg("wobbly.gridResolution", 8) + " · max vertices " + root.service.cfg("wobbly.maxVertices", 1024); color: Color.muted; font.pixelSize: Style.font.caption }
          Slider { width: parent.width; from: 4; to: 16; value: root.service.cfg("wobbly.gridResolution", 8); enabled: root.service.statusObject().wobbly.available; onMoved: root.service.setConfig("wobbly.gridResolution", Math.round(value)); }
          ActionButton { width: parent.width; text: "Disable Wobbly on battery"; checked: root.service.cfg("effects.disableOnBattery", true); onClicked: root.toggle("effects.disableOnBattery") }
          ActionButton { width: parent.width; text: "Disable Wobbly fullscreen"; checked: root.service.cfg("effects.disableOnFullscreen", true); onClicked: root.toggle("effects.disableOnFullscreen") }
